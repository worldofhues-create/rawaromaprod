...
git_sha(){ git -c safe.directory="$1" -C "$1" rev-parse HEAD 2>/dev/null || current GIT_SHA "$2"; }
...
app)
  ...
  # The app role is NOT granted /rawaroma/vault/*, by design: the KMS key id (not a secret) is carried.
  KMS=$(carry /rawaroma/vault/FORMULA_KMS_KEY_ID FORMULA_KMS_KEY_ID /etc/rawprod/api.env)
