...
> - ✅ **Admin + inventory MED** (2026-07-10, commit e644e01): **create-user** now hashes a plaintext password server-side (Argon2id) + a New-user form — users were seed-only; completes create→assign-role→revoke. **Inventory over-reserve** (can't reserve > on-hand − active reservations) + **over-issue** guards (transaction OUT + adjustment can't take on-hand negative). Live-verified 6/6.
> - ✅ **Vault hardening #6** (2026-07-10, commit 6ff3950): formula.audit_events **append-only DB trigger** (UPDATE/DELETE blocked — verified), owner-only **chain-verification endpoint** `GET /v1/formula-audit-verify` (recomputes the KEK-HMAC chain — verified ok:true over 14 real rows), and **ra_vault role-isolation** script + runbook (`scripts/provision-vault-isolation.sql`, `docs/VAULT_HARDENING.md` — surgical revoke of main-role access to the ciphertext tables; delivered, applied deliberately not auto). Deferred: `FORMULA_ACCESS_POLICY` non-owner scoped grants (design-dependent Phase-2).
> - ✅ **Guards & governance batch** (2026-07-10, commits bce4094+363ac1c): **G/#3** procurement state machine (PR submit only DRAFT / approve only SUBMITTED; PO approve only DRAFT / issue only APPROVED / acknowledge only ISSUED — 409 otherwise), **G/#9** dispatch guard subtracts consumed + rejects non-positive qty + aggregates per batch + closes the dispatch-items bypass, **G/#11** relay export transactional + genesis-first-import, **G/#1** RBAC revoke (delete user-role/role-permission mappings, owner-guarded). **Live-verified 6/6**: issue-unapproved-PO / ack-unissued-PO / approve-unsubmitted-PR all 409; grant→revoke removes the mapping.
> - ✅ **Write-correctness batch — 9 of 9 FIXED & verified** (2026-07-10): W1 inventory ledger — H-C1 transfer moves stock, H-C2 sign by movement kind, H-C4 release idempotent (commit 41130d9); W2 QC gating — H-I2 packaging-QC-FAIL blocks ATP+dispatch, H-I1 RM-QC REJECT/HOLD blocks availability, H-I3 REWORK emits+notifies (commit e09beaa); W3 production — H-C5 order no longer a dead-end (PLANNING→INPROGRESS), H-C6 guarded oil-batch state machine (commit 8e0dec2); **H-C3** production consumption → RM on-hand decrement via an idempotent inventory worker subscriber (commit 69e50a5) — **live-verified**: throwaway batch 1000→990 on issue, ledger + event-history written, re-emit no double-decrement; migration backfills existing issues so history is never retroactively decremented.
> - ✅ **MED + LOW backlog — FIXED & live-verified** (2026-07-10, commits 2b68959, ae41798, 290bb87): **#10 notify delivery-assurance** — `notification_log` gains `attempts`/`updated_dt`, the drain retries failures up to 3× and a dead-letter scan alerts on stuck events (no more silently-dropped emails). **#8 production-QC spec evaluation** — `recordProductionQc` now auto-grades PASS/FAIL against `[specMin, specMax]` (live: observed 5 in [1,10] → **PASS**, observed 20 → **FAIL**). **#7 CAPA / dispose idempotency** — a dispositioned inspection can't be re-dispositioned (live: 1st ACCEPT **201**, re-dispose **409**); CAPA registered in the generic edit registry. **#4 approval-matrix seeded** (`scripts/seed-approval-matrix.cjs`, in `db:provision`) — **full routing enforcement is honestly Phase-2** (matrix authorities are finer-grained than the 10 RBAC roles). **LOW:** login rate-limit/lockout (5 fails → 15-min 429; live-verified with a nonexistent identifier so no real user is locked), un-spoofable actor fields (`disposedBy`/`retainedBy` = authenticated principal, never a body field), and a `soNumber` collision fix (base36 full-epoch, replacing the last-5-digits form that collided every ~100s). **Deferred to Phase-2 (documented, non-blocking):** refresh-token reuse-detection (needs a session store), GEO `updateRegion` null-parent, seeding the 12 empty masters (trivial via existing create routes), full approval-routing enforcement, and `FORMULA_ACCESS_POLICY` non-owner grants.
>
> **→ All three Step-3 blockers (reproducibility, security, relay-apply) are cleared. Step 3 (offline console + FileKmsAdapter) is unblocked. Every HIGH audit item + all actionable MED + LOW polish are now closed and live-verified; the remaining open scope is net-new (④ storefront, ⑤ AI layer) plus the explicitly-deferred Phase-2 list above.**
