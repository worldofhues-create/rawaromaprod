...
**Vault / KmsPort seam.** `cluster-formula/src/vault.service.ts` is the single decrypt chokepoint. Ingredients are AES-GCM sealed under a per-formula DEK wrapped by the KEK via `KMS_PORT` (`crypto/kms.port.ts`; Phase-1 `EnvKmsAdapter` from `FORMULA_KEK`; swap-in point for real KMS/HSM). Every decrypt is APPROVED-only and writes a hash-chained audit row (advisory-lock-serialized) in the SAME tx — rolls back if the audit insert fails. Real `material_id` NEVER leaves the vault except via `getPickList` (server-side, no route) or the owner/policy-gated `getActualFormula`.
