...
**Make-to-ship chain:**
- **cluster-formula** (formula, vault) — **the crown jewel.** `vault.service.ts::decryptVersion()` (APPROVED-only else Forbidden) + `writeAudit()` (advisory-lock-serialized, hash-chained). `crypto/kms.port.ts` (`KMS_PORT` — `wrapDek/unwrapDek`; `EnvKmsAdapter` reads `FORMULA_KEK`). Events `formula.version.approved`, `formula.copy.approved`. Port `FORMULA_LOOKUP` (`getFloorView` alias-masked; `resolveManufacturingInstruction` floor code + quantity; `resolvePickList` / `resolveManufacturingLines` keyed material reference (`material-ref.ts`) + quantity — never material_id). The main app box reaches the Vault only through `VaultApiClient` (`vault-port.ts`, signed internal HTTP to vault-api; `VAULT_PORT` = cluster-production's `ProductionVaultPort`, which resolves the keyed references against the main DB's masterdata) and never composes `FormulaModule` or opens a formula-DB connection. Its non-recipe formula reads (dashboard runs/formula stage/feed, the finished-good trace, `/v1/formula-access-audit`) go through `VaultApiClient.formulaLabels` / `.accessAudit` (`formula-directory.service.ts` on the Vault: formula code + version number + status + event type only, never the formula name or an approver's remarks). The Vault console's material picker searches `MaterialCatalogue` (`facts-bridge/material-catalogue.ts`, in memory), which the main box's worker PUSHES (`backend/api/src/vault-bridge/material-catalogue-sync.service.ts`, id/code/name, digest probe every `VAULT_CATALOGUE_SYNC_MS`); the Vault never calls the main box (no `MAIN_API_INTERNAL_URL`).
...
| `dashboard` | `GET /v1/dashboard`,`/v1/alerts`,`/v1/notifications`,`/v1/trace/finished-good/:id` | Big cross-cluster read; already sums `finished_good_batch_master.produced_qty` (fgAgg) + joins `dispatch_master`; self-masks. Best FG-side join reference. Never joins `formula.*` (not in this DB): formula codes/statuses come from the Vault (`VaultApiClient.formulaLabels`, degrades to none). |
...
| `audit` | `/v1/formula-access-audit`, `/v1/login-history` | formula-access-audit = the Vault's `formula.audit_events` page over the signed channel + emails from `iam.user_master`; the Vault box serves the same route to the Vault console from its own chain. |
