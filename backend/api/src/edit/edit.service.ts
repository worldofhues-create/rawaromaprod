...
const REGISTRY: Record<string, ResourceCfg> = {
  ...
  // Lane fread-rp: formula_version lives in the Vault database, not this box's, so this UPDATE
  // could only ever 500 here ("relation formula.formula_version does not exist"). A version's
  // status moves only through the Vault's lifecycle routes on the Vault box (finalize / submit /
  // approve / reject / lock, with their segregation-of-duties checks) — a generic status PATCH
  // would bypass all of them even where the table did exist.
  'formula-versions': {
    schema: 'formula', table: 'formula_version', pk: 'formula_version_id', perm: 'formula:formula_version:write', cols: { status: 'status' },
    unavailable: 'Formula versions are not editable here: they live in the Formula Vault, and a version\'s status changes only through the Vault console\'s lifecycle actions (finalize, submit for review, approve, reject, lock).',
  },
