 ...
 * and compares it against how long ago the caller's credential was actually PROVED
 * (`principal.authTime`, set by `JwtAuthGuard` from the verified JWT — never client-supplied).
 ...
 * stale credential is asked to step up. Unmarked routes (no `@FreshAuth`) are unaffected.
 *
 * S3 SECURITY REVIEW ITEM 2 — WHY `authTime`, NOT `iat`. This guard used to read
 * `principal.iat` (token ISSUE time), which `AuthService.refresh()` re-mints fresh on every
 * refresh — so a session refreshed minutes ago always read as "fresh" regardless of how long
 * ago the caller actually presented a credential. `authTime` is carried forward unchanged
 * across every refresh (`JwtService.signRefresh`/`verifyRefresh`), so this guard now measures
 * the same thing for a refreshed session that it always measured for a freshly-logged-in one.
 *
 * Re-authenticating today means re-running `POST /auth/login` (the same password/OTP the
 * caller already has), or, via the ALEMBIC assertion bridge, a fresh step-up on that side —
 * either mints a token with a new `authTime`. This launch does not add a second MFA factor
 * (§109.5: "do not reintroduce an ordinary TOTP/MFA wizard to all staff").
 */
import {
  type CanActivate,
...
export class FreshAuthGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // ever letting a real backdated/forged `authTime` read as fresh.
    if (!Number.isFinite(user.authTime) || ageSeconds > maxAgeSeconds || ageSeconds < -5) {
