...
(function () {
   ...
   * THE VAULT SENDS LESS. Only the question and the console name: never the view (which Vault
   * screen someone is on is Vault context), never a formula, and no thread is kept in ALEMBIC
   * (persist is off here) — the server drops the view again on both sides regardless. The call
   * goes over the MAIN_API channel, the same one this console already signs in through: the
   * bridge secret lives on the main app box, never on the Vault box.
  ...
  var ariaApi = null, ariaLoading = null, ariaCtx = 'Vault', ariaMode = null, ariaMsg = '';
  function ariaView() { var i = ariaCtx.indexOf(' · '); return i < 0 ? 'Vault' : ariaCtx.slice(i + 3); }
  function loadAria() {
  ...
  function ariaStatus() {
    ...
    return mainTunnel('/v1/aria/status').then(function (r) {
  ...
  function ariaAsk(question, extra) {
    var body = { question: question, console: 'vault', persist: false };
    if (extra && extra.conversationId) body.conversationId = extra.conversationId;
    return mainTunnel('/v1/aria/ask', { method: 'POST', body: body }).then(function (r) {
  ...
  function renderShell(activeView, contentEl) {
    ...
    var rail = h('nav', { class: 'rail', id: 'pv-rail', 'aria-label': 'Vault navigation' }, [
      ...
      h('div', { class: 'rme' }, [
        ...
        h('button', { type: 'button', class: 'rail-min', style: 'position:static;margin-left:0', onclick: function () { railOpen = false; ariaReset(); document.body.classList.remove('rail-off', 'rail-open', 'dock-away'); logout(); }, 'aria-label': 'Sign out', title: 'Sign out' }, [icon(ICONS.logout, 13)]),
      ]),
    ]);
